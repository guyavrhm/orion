...
import { STREAMS_DIR } from '../../src/main/utils/paths.js';
...
describe('Stream & Subtitle Routes Integration Tests', () => {
  ...
  const movieHlsDir = path.join(STREAMS_DIR, 'movies', testMovieId, 'hls');
  const movieSubDir = path.join(STREAMS_DIR, 'movies', testMovieId, 'subtitles');
  const showHlsDir = path.join(STREAMS_DIR, 'shows', testShowId, '1', '1', 'hls');
  const showSubDir = path.join(STREAMS_DIR, 'shows', testShowId, '1', '1', 'subtitles');

  const movieBaseDir = path.join(STREAMS_DIR, 'movies', testMovieId);
  const showBaseDir = path.join(STREAMS_DIR, 'shows', testShowId);
...
Hello, world!`;
  ...
  // ==========================================
  // 2. GET /stream/subtitles/:id/:filename
  // ==========================================
  describe('GET /stream/subtitles/:id/:filename', () => {
    it('serves .vtt subtitle file via /stream/subtitles with Content-Type text/vtt and CORS headers', async () => {
      const res = await request(app).get(`/stream/subtitles/${testMovieId}/en.vtt`);
    ...
    it('serves show episode subtitle file via composite id (tt_s1_e1)', async () => {
      const res = await request(app).get(`/stream/subtitles/${testShowFileId}/es.vtt`);
    ...
    it('returns 404 when subtitle file does not exist', async () => {
      const res = await request(app).get(`/stream/subtitles/${testMovieId}/fr.vtt`);
    ...
    it('returns 404 when fileId does not exist', async () => {
      const res = await request(app).get('/stream/subtitles/tt0000000/en.vtt');
  ...
  describe('Path Traversal & Security Validation', () => {
    ...
    it('prevents directory traversal using ../../ in filename for /subtitles', async () => {
      const res = await request(app).get(`/stream/subtitles/${testMovieId}/../../../../etc/passwd`);
    ...
    it('prevents directory traversal using URL-encoded slashes in /subtitles', async () => {
      const res = await request(app).get(`/stream/subtitles/${testMovieId}/%2e%2e%2f%2e%2e%2fetc%2fpasswd`);
    ...
    it('rejects invalid fileId formats with special characters safely without erroring', async () => {
      ...
      for (const id of invalidIds) {
        ...
        const subRes = await request(app).get(`/stream/subtitles/${encodeURIComponent(id)}/en.vtt`);
