  ...
  describe('getLocalSubtitles Scanning', () => {
    ...
    it('should discover local subtitle files and format track URLs', () => {
      ...
      expect(tracks[0]).toEqual({
        lang: 'eng',
        url: `/stream/subtitles/${fileId}/${vttFileName}`
      });
    });
    ...
    it('should read scores.json and pick the highest scoring subtitle track for a language', () => {
      ...
      expect(engTrack?.url).toBe(`/stream/subtitles/${fileId}/${track2}`);

      expect(spaTrack).toBeDefined();
      expect(spaTrack?.url).toBe(`/stream/subtitles/${fileId}/${track3}`);
    });

    it('should handle corrupt scores.json gracefully and return valid subtitle tracks', () => {
      fs.mkdirSync(realSubsDir, { recursive: true });
      ...
      expect(tracks[0]).toEqual({
        lang: 'eng',
        url: `/stream/subtitles/${fileId}/${track1}`
      });
    });
