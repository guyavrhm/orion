...
describe('OpenSubtitlesClient', () => {
  ...
  beforeEach(() => {
    client = new OpenSubtitlesClient(2, 0);
    mockFetch = vi.fn();
  ...
  describe('fetchSubtitles by hash and size (searchSubtitlesByHash)', () => {
    ...
    it('should format show endpoint with season and episode when searching by hash', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({
          subtitles: [
            {
              id: 'sub-show-1',
              lang: 'eng',
              url: 'https://dl.opensubtitles.org/show1.srt'
            }
          ]
        })
      });
      ...
      expect(result[0].id).toBe('sub-show-1');
  ...
  describe('Response normalization and error / rate limit handling', () => {
    ...
    it('should return empty array on 404 Not Found response without retrying', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 404,
    ...
    it('should return empty array on network failure after all retries exhausted without throwing', async () => {
    ...
    it('should retry on network failure and succeed on subsequent attempt', async () => {
      ...
      expect(results[0].id).toBe('sub-recovered');
      expect(mockFetch).toHaveBeenCalledTimes(2);
    });

    it('should retry on transient HTTP 502/503/429 and succeed on subsequent attempt', async () => {
      mockFetch
        .mockResolvedValueOnce({
          ok: false,
          status: 502,
      ...
      const results = await client.fetchSubtitles('tt0111161', 'tt0111161', 'movie');
      expect(results).toHaveLength(1);
      expect(results[0].id).toBe('sub-502-recovered');
      expect(mockFetch).toHaveBeenCalledTimes(2);
    });

    it('should return empty array on persistent HTTP 5xx after retries are exhausted without throwing', async () => {
