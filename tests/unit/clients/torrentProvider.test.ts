...
describe('TorrentProviderClient & Utilities', () => {
  ...
  describe('flattenNormalizedKeys', () => {
    it('should normalize top-level keys removing dashes, underscores and casing', () => {
      const raw = { Video_Codec: 'x264', 'release-title': 'Movie.2026', FILE_IDX: 2 };
      const norm = flattenNormalizedKeys(raw);
      expect(norm).toEqual({
        videocodec: 'x264',
    ...
    it('should recursively extract keys from nested objects', () => {
      const raw = {
        title: 'Night of the Living Dead 1968 1080p',
        nested: {
          filename: 'Night.of.the.Living.Dead.1968.1080p.WEBRip.x264.mp4',
        }
      };
      const norm = flattenNormalizedKeys(raw);
      expect(norm.title).toBe('Night of the Living Dead 1968 1080p');
      expect(norm.filename).toBe('Night.of.the.Living.Dead.1968.1080p.WEBRip.x264.mp4');
    });

    it('should handle null, undefined, arrays, and non-objects gracefully', () => {
      expect(flattenNormalizedKeys(null)).toEqual({});
      expect(flattenNormalizedKeys(undefined)).toEqual({});
      expect(flattenNormalizedKeys('string')).toEqual({});
      expect(flattenNormalizedKeys([1, 2, 3])).toEqual({});
    });
  });
  ...
  describe('parseCodec', () => {
    ...
    it('should detect codec from nested nested.filename', () => {
      const raw = {
        name: 'Provider\n1080p',
        title: 'Night of the Living Dead 1968 1080p WEBRip 👤 1380 💾 1.95 GB ⚙️ ReleaseGroup',
        nested: {
          filename: 'Night.of.the.Living.Dead.1968.1080p.WEBRip.x264.AAC5.1-[ReleaseGroup].mp4'
        }
      };
      expect(client.parseCodec(raw)).toBe('h264');
    });
  ...
  describe('extractStreamsFromResponse', () => {
    ...
    it('should extract streams from Torznab / RSS XML feed', () => {
      ...
      <rss version="2.0" xmlns:torznab="http://torznab.com/schemas/2015/feed">
        <channel>
          ...
          <item>
            <title>Night.of.the.Living.Dead.1968.1080p.BluRay.x264</title>
            <guid>item-1</guid>
      ...
      expect(streams).toHaveLength(1);
      expect(streams[0].hash).toBe(validHash);
      expect(streams[0].title).toBe('Night.of.the.Living.Dead.1968.1080p.BluRay.x264');
      expect(streams[0].quality).toBe('1080p');
    ...
    it('should filter streams by season and episode when specified', () => {
      ...
      const s1e1 = client.extractStreamsFromResponse(showResponse, 1, 1);
      expect(s1e1).toHaveLength(1);
      expect(s1e1[0].hash).toBe('1111111111111111111111111111111111111111');

      const s1e2 = client.extractStreamsFromResponse(showResponse, 1, 2);
  ...
  describe('buildProviderUrl', () => {
    it('should replace all placeholders properly for movie templates', () => {
      const template = 'https://provider.org/api?id={imdbId}&num={imdbNumericId}&q={title}&page={page}&key={apiKey}';
      const url = client.buildProviderUrl(template, 'tt0056923', 'movie', undefined, undefined, 'Charade', 2, 'secret123');

      expect(url).toBe('https://provider.org/api?id=tt0056923&num=0056923&q=Charade&page=2&key=secret123');
    });

    it('should replace season, seasonPad, episode, episodePad placeholders for show templates', () => {
      const template = 'https://provider.org/show?id={imdbId}&s={season}&sp={seasonPad}&e={episode}&ep={episodePad}';
      const url = client.buildProviderUrl(template, 'tt0055662', 'show', 3, 7);
  ...
  describe('getProviderTemplates and getProviderApiKey', () => {
    ...
    it('should support singular TORRENT_MOVIE_PROVIDER or TORRENT_SHOW_PROVIDER', () => {
      ...
      expect(client.getProviderTemplates('show')).toEqual(['https://single.org/shows']);
    });
  ...
  describe('filterAndRankTorrents', () => {
    ...
    it('should yield H264 to HEVC only when H264 has modest peers (< 30) and HEVC has overwhelming peers (>= 50 and >= 5x)', () => {
      ...
      const ranked = client.filterAndRankTorrents(streams, 'movie', 3);
      expect(ranked[0].hash).toBe('hevc');
  ...
  describe('getTorrentStreams & getTopTorrents end-to-end', () => {
    ...
    it('should query show torrents with season & episode and return top ranked streams', async () => {
      ...
      const top = await client.getTopTorrents('tt0055662', 'show', 1, 1, 2);
