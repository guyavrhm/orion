...
describe('MetahubClient', () => {
  ...
  describe('searchMetahub', () => {
    it('should search for media and return matched items', async () => {
      ...
      expect(results[0].title).toBe('Night of the Living Dead');
      expect(results[0].year).toBe('1968');
      expect(results[0].genres).toEqual(['Horror']);
      expect(results[1].id).toBe('tt0052077');
      expect(results[1].title).toBe('Plan 9 from Outer Space');
    });

    it('should correctly parse and normalize both movie and series types in results', async () => {
      ...
      expect(results).toHaveLength(2);
      const showItem = results.find(item => item.type === 'show');
