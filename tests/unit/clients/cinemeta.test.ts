...
describe('CinemetaClient', () => {
  ...
  describe('fetchPopularMovies (getPopularMovies)', () => {
    it('should fetch and return popular movies from Cinemeta catalogs API', async () => {
      ...
      expect(result[0].title).toBe('Night of the Living Dead');
  ...
  describe('fetchPopularShows', () => {
    it('should fetch and return popular TV shows catalog', async () => {
      const mockShows = [
        {
          id: 'tt0055662',
      ...
      expect(result[0].id).toBe('tt0055662');
      expect(result[0].title).toBe('The Beverly Hillbillies');
      expect(result[0].type).toBe('show');
      expect(result[0].rating).toBe('7.2');
    });
  ...
  describe('fetchMetadataDetails (getMediaDetails)', () => {
    it('should fetch and normalize movie metadata details correctly', async () => {
      ...
      expect(result.title).toBe('Charade');
      expect(result.rating).toBe('7.9');
      expect(result.cast).toHaveLength(3);
    });

    it('should fetch and normalize show metadata details with episodes', async () => {
      const rawMeta = {
        id: 'tt0032475',
        type: 'series',
      ...
      const result = (await client.fetchMetadataDetails('tt0032475', 'show')) as any;
      ...
      expect(result.title).toBe('Flash Gordon Conquers the Universe');
      // genre normalized to genres
      expect(result.genres).toEqual(['Action', 'Adventure', 'Sci-Fi']);
      expect(result.episodes).toHaveLength(2);
      expect(result.episodes[0].title).toBe('The Purple Death');
      expect(result.episodes[1].title).toBe('Freezing Torture');
    });
    ...
    it('should normalize ID from imdb_id, id, or requested parameter fallback', async () => {
      ...
      expect(result.type).toBe('movie');
      expect(result.title).toBe('No ID Movie');
