...
import { QUEUE_NAMES, DOWNLOAD_WATCHDOG, DOWNLOAD_PRIORITIES } from '../../../src/main/config/queue.js';
...
describe('downloadWorker', () => {
  ...
  describe('processDownloadJob', () => {
    it('skips download and returns already_downloaded when master playlist already exists', async () => {
      ...
      const publishSpy = vi.spyOn(queues, 'publishMediaRequestStatus').mockResolvedValue(1);
      ...
      expect(publishSpy).toHaveBeenCalledWith('tt1111111', MEDIA_REQUEST_STATUS.READY);
    });
    ...
    it('runs eviction free space check before downloading and fails if all candidates fail', async () => {
      ...
      const job = {
        ...
        data: {
          ...
          candidates: [
            { hash: 'hash_available', magnetUrl: 'magnet:?xt=urn:btih:hash_available', fileIdx: 0, quality: '1080p', size_bytes: 5000, title: 'avail' }
          ],
          failedCandidates: []
        }
      } as unknown as Job;
    ...
    it('skips tournament and proceeds directly when only 1 candidate remains', async () => {
      vi.spyOn(helpers, 'getMediaDirs').mockReturnValue({
        baseDir: '/downloads/movies/tt_single',
        subtitlesDir: '/downloads/movies/tt_single/subtitles',
        hlsDir: '/downloads/movies/tt_single/hls'
      });
      vi.spyOn(fs, 'existsSync').mockReturnValue(false);
      vi.spyOn(fs, 'mkdirSync').mockReturnValue(undefined as unknown as string);
      vi.spyOn(eviction, 'ensureFreeSpace').mockImplementation(() => {});
      vi.spyOn(hashUtil, 'calculateOSHash').mockResolvedValue({ hash: 'os_single', size: 1000 });
      vi.spyOn(queues, 'enqueueMediaProcessingFlow').mockResolvedValue({} as never);

      class SingleTorrent extends EventEmitter {
        name = 'SingleTorrent';
        path = '/tmp/downloads';
        ready = true;
        ...
        downloaded = 1000;
        progress = 1;
        files = [{ name: 'Single.mkv', path: 'Single.mkv', length: 1000, downloaded: 1000, select: vi.fn(), deselect: vi.fn() }];
        destroy = vi.fn().mockImplementation((cb) => { if (cb) cb(); });
      }
      const singleTorrent = new SingleTorrent();
      activeMockTorrent = singleTorrent;

      const job = {
        id: 'job-single-candidate',
        data: {
          fileId: 'tt_single',
          type: 'movie',
          quality: '1080p',
          ...
          candidates: [
            { hash: 'single_hash', magnetUrl: 'magnet:?xt=urn:btih:single_hash', fileIdx: 0, quality: '1080p', size_bytes: 1000, title: 'single' }
          ],
          failedCandidates: []
        },
      ...
      const result = await processDownloadJob(job as never);
      expect(result.fileId).toBe('tt_single');
      expect(result.status).toBe('completed');
    });

    it('fails over on torrent error and re-enqueues job with lockedCandidateHash: undefined to trigger fresh tournament', async () => {
      vi.spyOn(helpers, 'getMediaDirs').mockReturnValue({
        baseDir: '/downloads/movies/tt_failover',
        subtitlesDir: '/downloads/movies/tt_failover/subtitles',
        hlsDir: '/downloads/movies/tt_failover/hls'
      });
      vi.spyOn(fs, 'existsSync').mockReturnValue(false);
      vi.spyOn(eviction, 'ensureFreeSpace').mockImplementation(() => {});

      const addSpy = vi.spyOn(queues.downloadQueue, 'add').mockResolvedValue({ id: 'new-job' } as never);
      ...
      const job = {
        id: 'job-download-failover',
        data: {
          fileId: 'tt_failover',
          type: 'movie',
          quality: '1080p',
          lockedCandidateHash: 'hash_1',
          candidates: [
            { hash: 'hash_1', magnetUrl: 'magnet:?xt=urn:btih:hash_1', fileIdx: 0, quality: '1080p', size_bytes: 5000, title: 'c1' },
            { hash: 'hash_2', magnetUrl: 'magnet:?xt=urn:btih:hash_2', fileIdx: 0, quality: '720p', size_bytes: 3000, title: 'c2' }
          ],
          failedCandidates: []
        }
      } as unknown as Job;
      ...
      setTimeout(() => {
        errTorrent.emit('error', new Error('Tracker timeout'));
      }, 10);

      const result = await promise;
      expect(result).toEqual({ fileId: 'tt_failover', status: 'failover' });
      expect(addSpy).toHaveBeenCalledWith(
        'download',
        expect.objectContaining({
          fileId: 'tt_failover',
          lockedCandidateHash: undefined,
          failedCandidates: ['hash_1']
        }),
        { priority: DOWNLOAD_PRIORITIES.HIGH }
      );
    });

    it('fails over on dead stream timeout (DEAD_PRUNE_SEC) and re-enqueues job with lockedCandidateHash: undefined', async () => {
      vi.useFakeTimers();

      vi.spyOn(helpers, 'getMediaDirs').mockReturnValue({
        baseDir: '/downloads/movies/tt_dead',
        subtitlesDir: '/downloads/movies/tt_dead/subtitles',
        hlsDir: '/downloads/movies/tt_dead/hls'
      });
      vi.spyOn(fs, 'existsSync').mockReturnValue(false);
      vi.spyOn(eviction, 'ensureFreeSpace').mockImplementation(() => {});

      const addSpy = vi.spyOn(queues.downloadQueue, 'add').mockResolvedValue({ id: 'new-dead-job' } as never);

      class DeadTorrent extends EventEmitter {
        name = 'DeadTorrent';
        path = '/tmp/downloads';
        ready = true;
        downloadSpeed = 0;
        downloaded = 0;
        progress = 0;
        files = [{ name: 'Dead.mkv', length: 5000, downloaded: 0, select: vi.fn(), deselect: vi.fn() }];
        destroy = vi.fn().mockImplementation((cb) => { if (cb) cb(); });
      }
      activeMockTorrent = new DeadTorrent();

      const job = {
        id: 'job-download-dead',
        data: {
          fileId: 'tt_dead',
          type: 'movie',
          quality: '1080p',
          lockedCandidateHash: 'hash_dead_1',
          candidates: [
            { hash: 'hash_dead_1', magnetUrl: 'magnet:?xt=urn:btih:hash_dead_1', fileIdx: 0, quality: '1080p', size_bytes: 5000, title: 'c1' },
            { hash: 'hash_dead_2', magnetUrl: 'magnet:?xt=urn:btih:hash_dead_2', fileIdx: 0, quality: '720p', size_bytes: 3000, title: 'c2' }
          ],
          failedCandidates: []
        },
        updateProgress: vi.fn().mockResolvedValue(undefined)
      ...
      // Advance by DEAD_PRUNE_SEC * 1000 + buffer to trigger dead failover
      await vi.advanceTimersByTimeAsync(DOWNLOAD_WATCHDOG.DEAD_PRUNE_SEC * 1000 + 2000);

      const result = await promise;
      expect(result).toEqual({ fileId: 'tt_dead', status: 'failover' });
      expect(addSpy).toHaveBeenCalledWith(
        'download',
        expect.objectContaining({
          fileId: 'tt_dead',
          lockedCandidateHash: undefined,
          failedCandidates: ['hash_dead_1']
        }),
        { priority: DOWNLOAD_PRIORITIES.HIGH }
      );
    ...
    it('completes download, computes OSHash, and enqueues media processing flow', async () => {
      ...
      await vi.advanceTimersByTimeAsync(DOWNLOAD_WATCHDOG.POLL_INTERVAL_MS + 100);
      const result = await processPromise;

      expect(result.fileId).toBe('tt3333333');
      expect(result.status).toBe('completed');
      expect(enqueueFlowSpy).toHaveBeenCalledWith(
        expect.objectContaining({
          fileId: 'tt3333333',
          rawTempDir: expect.stringContaining('tt3333333')
        })
      );
