...
describe('transcodeWorker', () => {
  ...
  describe('probeMediaStreams', () => {
    ...
    it('identifies H.264 4:2:2 profile as non-copyable (isVideoH264: false)', async () => {
      const mockFfprobeData = {
        streams: [
          {
            codec_type: 'video',
    ...
    it('ignores attached_pic cover art stream and probes primary movie stream', async () => {
      const mockFfprobeData = {
        streams: [
          {
            codec_type: 'video',
      ...
      const result = await probeMediaStreams('/path/to/movie_with_cover.mp4', ['eng']);
      expect(result.isVideoH264).toBe(true);
      expect(result.videoCodec).toBe('h264');
  ...
  describe('convertToHls', () => {
    ...
    it('generates Fast Lane parameters (-c:v copy + aac) when video is standard 8-bit H.264', async () => {
      ...
      expect(outputOpts).toContain('-bsf:v h264_mp4toannexb');
      ...
      expect(outputOpts).toContain('-af aresample=async=1:first_pts=0');
    ...
    it('generates Heavy Lane parameters (libx264 veryfast crf 22) when video is HEVC/10-bit', async () => {
      ...
      expect(publishStatusSpy).toHaveBeenCalledWith('tt9999999', MEDIA_REQUEST_STATUS.PROCESSING, '0.00');
    });
    ...
    it('throttles and broadcasts progress percentage updates', async () => {
      ...
      expect(publishStatusSpy).toHaveBeenCalledWith('tt222', MEDIA_REQUEST_STATUS.PROCESSING, '15.00');
      expect(publishStatusSpy).toHaveBeenCalledWith('tt222', MEDIA_REQUEST_STATUS.PROCESSING, '100.00');
    });
  ...
  describe('processTranscodeJob', () => {
    ...
    it('throws error and cleans up partial HLS chunks if transcoding fails', async () => {
      vi.spyOn(helpers, 'getMediaDirs').mockReturnValue({
      ...
      expect(publishStatusSpy).not.toHaveBeenCalledWith('tt9999999', MEDIA_REQUEST_STATUS.FAILED);
      expect(rmSyncSpy).toHaveBeenCalledWith(expect.stringContaining('hls'), { recursive: true, force: true });
    });
