...
describe('subtitleWorker', () => {
  ...
  describe('calculateSubtitleScore', () => {
    ...
    it('scores resolution and source matches when release group is absent', () => {
      ...
      const score = calculateSubtitleScore('Show.S01E01.720p.WEB-DL.srt', videoMeta);
...
`;
    ...
    it('scoreLocalSubtitles extracts padded reference audio with first_pts=0 before scoring', async () => {
      vi.spyOn(helpers, 'getMediaDirs').mockReturnValue({
        baseDir: '/media/movies/tt1234567',
        subtitlesDir: '/media/movies/tt1234567/subtitles',
        hlsDir: '/media/movies/tt1234567/hls'
      ...
      const execArgsList: string[][] = [];
      vi.spyOn(childProcess, 'execFile').mockImplementation((_file, args, opts, callback) => {
        execArgsList.push((args as string[]) || []);
        const cb = typeof opts === 'function' ? opts : callback;
        if (cb) {
          cb(null, '', 'score: 125000\noffset seconds: 0.15');
        }
        return { pid: 9999 } as unknown as childProcess.ChildProcess;
      });
      ...
      // Check ffmpeg extracted audio with first_pts=0
      const ffmpegCall = execArgsList.find((args) => args.includes('aresample=async=1:first_pts=0'));
      expect(ffmpegCall).toBeDefined();
      expect(ffmpegCall).toContain('-threads');
      expect(ffmpegCall).toContain('1');
      expect(ffmpegCall).toContain('-vn');
      expect(ffmpegCall).toContain('-ac');
      expect(ffmpegCall).toContain('1');
    });
  });
  ...
  describe('processSubtitleJob and Worker', () => {
    ...
    it('throws if subtitle processing fails', async () => {
      vi.spyOn(helpers, 'getMediaDirs').mockImplementation(() => {
      ...
      const job = {
        ...
        data: {
          fileId: 'tt1234567',
          sourcePath: '/media/corrupt.mp4'
        }
      } as unknown as Job;

      await expect(processSubtitleJob(job as never)).rejects.toThrow('Fatal filesystem error');
    });
