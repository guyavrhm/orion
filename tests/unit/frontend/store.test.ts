...
describe('Store (Frontend State Management)', () => {
  ...
  describe('cacheMetadata', () => {
    ...
    it('should merge partial metadata without overwriting existing complete fields', () => {
      ...
      store.cacheMetadata({
        ...
        rating: '7.9',
        runtime: '113 min',
        title: undefined, // Should NOT overwrite existing title
    ...
    it('should safely merge videos/episodes list by season & episode without duplicates', () => {
      store.cacheMetadata({
        ...
        episodes: [
          { season: 1, episode: 1, title: 'The Clampetts Strike Oil', thumbnail: 'thumb1.jpg' },
          { season: 1, episode: 2, title: 'Getting Settled', thumbnail: 'thumb2.jpg' }
        ]
      });
      ...
      store.cacheMetadata({
        ...
        episodes: [
          ...
          { season: 1, episode: 3, title: 'Meanwhile, Back at the Cabin', thumbnail: 'thumb3.jpg' }
        ]
      });

      const cached = store.getMedia('tt0055662');
      expect(cached?.episodes).toHaveLength(3);

      // Episode 1 should retain original title and thumbnail while gaining new overview
      const ep1 = cached?.episodes.find((v: any) => v.season === 1 && v.episode === 1);
      expect(ep1?.title).toBe('The Clampetts Strike Oil');
      expect(ep1?.thumbnail).toBe('thumb1.jpg');
      expect(ep1?.overview).toBe('Updated Pilot description');

      // Episode 3 should be newly added
      const ep3 = cached?.episodes.find((v: any) => v.season === 1 && v.episode === 3);
      expect(ep3?.title).toBe('Meanwhile, Back at the Cabin');
    });

    it('should strip transient properties progress and is_ready from metadata', () => {
      store.cacheMetadata({
        id: 'tt0063350',
        title: 'Night of the Living Dead',
      ...
      expect(cached?.is_ready).toBeUndefined();
    });
  ...
  describe('Watch Progress and Continue Watching', () => {
    ...
    it('should update local progress cache and continue watching list for shows', () => {
      store.updateLocalProgressCache({
        showId: 'tt0903747',
        timestamp: 600,
    ...
    it('should compute getMovieConfig with progress and ready status', () => {
      store.state.progress['tt0111161'] = { id: 'tt0111161', timestamp: 2500 } as any;
      store.state.ready['tt0111161'] = { id: 'tt0111161', is_ready: true } as any;
    ...
    it('should compute getShowConfig aggregating episode progress and latest watched episode', () => {
      ...
      store.state.ready['tt0903747_s1_e1'] = { id: 'tt0903747_s1_e1', is_ready: true } as any;
      ...
      expect(showConfig.episodes['tt0903747_s1_e1'].is_ready).toBe(true);
      expect(showConfig.episodes['tt0903747_s1_e2'].is_ready).toBe(false);
    });
  });

  describe('Ready and Request State Management', () => {
    it('should track batch and individual ready records', () => {
      store.cacheReadyBatch([
        { id: 'tt0111161' },
        { id: 'tt0903747_s1_e1' }
      ]);

      expect(store.isMovieReady('tt0111161')).toBe(true);
      expect(store.isEpisodeReady('tt0903747', 1, 1)).toBe(true);
      expect(store.isEpisodeReady('tt0903747', 1, 2)).toBe(false);
    });

    it('should manage active in-flight request progress states', () => {
      store.setActiveMediaRequest('tt0111161', 'preparing', '45.50');
      expect(store.getActiveMediaRequest('tt0111161')).toEqual({
        fileId: 'tt0111161',
        status: 'preparing',
        progress: '45.50'
      });
      expect(store.isPreparingOrQueued('tt0111161')).toBeTruthy();

      store.removeActiveMediaRequest('tt0111161');
      expect(store.getActiveMediaRequest('tt0111161')).toBeNull();
    });

    it('should set ready status to true and clear active request record', () => {
      store.setActiveMediaRequest('tt0111161', 'preparing', '99.00');
      store.setMediaRequestStatus('tt0111161', true);

      expect(store.isMovieReady('tt0111161')).toBe(true);
      expect(store.getActiveMediaRequest('tt0111161')).toBeNull();

      // Setting request status to false removes it
      store.setMediaRequestStatus('tt0111161', false);
      expect(store.isMovieReady('tt0111161')).toBe(false);
    });
  });
});
