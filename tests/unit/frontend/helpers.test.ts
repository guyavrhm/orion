...
describe('Frontend Helpers (renderer/utils/helpers.js)', () => {
  ...
  describe('buildMetadataHtml', () => {
    ...
    it('should format show metadata with type Show and genre fallback', () => {
      const showItem = {
        type: 'show',
        genres: ['Drama'],
        year: '2008',
  ...
  describe('getMediaRequestErrorMessage', () => {
    it('should return custom descriptive error messages for known error codes', () => {
      expect(getMediaRequestErrorMessage(ErrorCode.PROVIDER_NOT_CONFIGURED))
        .toBe('Torrent provider is not configured. Please check server settings in .env.');

      expect(getMediaRequestErrorMessage(ErrorCode.PROVIDER_UNAVAILABLE))
        .toBe('Torrent provider is temporarily unavailable or timed out. Please try again later.');

      expect(getMediaRequestErrorMessage(ErrorCode.NO_STREAMS_FOUND))
        .toBe('No streams found for this title.');

      expect(getMediaRequestErrorMessage(ErrorCode.MEDIA_NOT_READY))
        .toBe('Content is not ready yet. Please request it first.');
    });

    it('should return generic error message for unknown or null error codes', () => {
      expect(getMediaRequestErrorMessage('UNKNOWN_ERROR_CODE' as any))
        .toBe('Failed to request media. Please try again later.');
      expect(getMediaRequestErrorMessage(null as any))
        .toBe('Failed to request media. Please try again later.');
    });
  });
