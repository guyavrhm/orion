...
class App {
  ...
  async init() {
    ...
    try {
      ...
      const activeMediaRequests = data.activeMediaRequests ?? {};
      store.updateState({ activeMediaRequests }, 'requests-changed');
    } catch (e) {
    ...
    bottomSheet.init((episode, show) => {
      const resolvedShow = show ?? store.getMedia(episode.show_id);
      if (resolvedShow) {
  ...
  setupEventListeners() {
    ...
    eventBus.on('start-last-episode', async (item) => {
      const config = store.getShowConfig(item.id);
      
      const season = item.season ?? config?.last_season ?? 1;
      const episodeNum = item.episode ?? config?.last_episode ?? 1;
      const epTitle = item.episodeTitle ?? config?.last_episode_title ?? `Episode ${episodeNum}`;
      const episodeId = `${item.id}_s${season}_e${episodeNum}`;
      const startTime = item.timestamp ?? config?.episodes[episodeId]?.timestamp ?? 0;
      
      playerView.open({
        title: `${item.title} - S${season}E${episodeNum}: ${epTitle}`,
        runtime: item.runtime
      }, startTime, false, null);

      const fullShow = await this.handleOpenShowDetails(item, false);
      const episode = fullShow.episodes.find(v => v.season === season && v.episode === episodeNum);
  ...
  setupStoreObserver() {
    store.subscribe((changeType, state, detail) => {
      ...
      if (changeType === 'request-progress-updated' && detail && detail.fileId) {
        document.querySelectorAll(`[data-file-id="${detail.fileId}"]`).forEach(el => {
          const type = el.getAttribute('data-type');
          MediaCard.updateRequestState(el, detail.fileId, type);
        });
        if (bottomSheet.currentItem) {
          bottomSheet.updateButtonsState();
        }
      } else if (['requests-changed', 'queue-changed'].includes(changeType)) {
        document.querySelectorAll('[data-file-id]').forEach(el => {
          const fileId = el.getAttribute('data-file-id');
          const type = el.getAttribute('data-type');
          MediaCard.updateRequestState(el, fileId, type);
        });
  ...
  async syncState() {
    ...
    console.log('[App] Synchronizing request and queue state with server...');
    try {
      const data = await apiService.fetchQueueState();
      const activeMediaRequests = data.activeMediaRequests ?? {};
      store.updateState({ activeMediaRequests }, 'requests-changed');
  ...
  async loadInitialContent() {
    try {
      await Promise.all([
        ...
        apiService.fetchContinueWatching('show', 10)
      ]);
      console.log('[App] Initial content cached successfully.');
    } catch (e) {
      console.warn('[App] Failed to load initial content:', e);
    }
  ...
  async handleOpenPlayer(movie) {
    ...
    const startTime = config?.timestamp ?? 0;
    ...
    try {
      ...
      if (!fullMovie) throw new Error('Invalid movie metadata');
      
      const title = document.getElementById('video-movie-title');
      if (title) title.innerText = fullMovie.title;
    } catch (e) {
      if (!playerView.isOpen) return;
      console.error(e);
    ...
    playerView.open(fullMovie, startTime, false);
  ...
  async handleOpenShowDetails(show, shouldShow = true) {
    const onEpisodeClick = (ep) => {
      this.handleOpenEpisode(show, ep);
    };
    ...
    try {
      const response = await apiService.fetchShowDetails(show.id);
      const fullShow = response.metadata;
      if (!fullShow) throw new Error('Invalid show metadata');
  ...
  handleStartFirstEpisode(show) {
    ...
    const s1 = show.episodes.filter(v => parseInt(v.season, 10) === 1);
    if (s1.length > 0) {
      s1.sort((a, b) => a.episode - b.episode);
      this.handleOpenEpisode(show, s1[0]);
    } else {
      const all = show.episodes.filter(v => parseInt(v.season, 10) > 0).sort((a, b) => {
  ...
  async handleOpenEpisode(show, episode) {
    ...
    const startTime = config?.episodes?.[episodeId]?.timestamp ?? 0;
    
    let hasNext = false;
    if (show.episodes) {
      const currentIndex = show.episodes.findIndex(v => v.season === episode.season && v.episode === epNum);
      if (currentIndex !== -1 && currentIndex < show.episodes.length - 1) {
        const next = show.episodes[currentIndex + 1];
        hasNext = parseInt(next.season, 10) > 0;
      }
    }
    ...
    playerView.open({
      title: `${show.title} - S${episode.season}E${epNum}: ${epTitle}`,
      runtime: episode.runtime || show.runtime
    }, startTime, hasNext);

    apiService.fetchShowDetails(show.id).catch(e => console.error('Background details fetch failed:', e));
  ...
  handleStartNextEpisode(show, currentEpisode) {
    ...
    const currentIndex = show.episodes.findIndex(v => 
      v.season === currentEpisode.season && v.episode === epNum
    );

    if (currentIndex !== -1 && currentIndex < show.episodes.length - 1) {
      const nextEpisode = show.episodes[currentIndex + 1];
      if (parseInt(nextEpisode.season, 10) > 0) {
        const nextEpNum = nextEpisode.episode;
        const isReady = store.isEpisodeReady(show.id, nextEpisode.season, nextEpNum);
