...
class SearchView {
  ...
  async executeSearch(query) {
    ...
    try {
      ...
      const filtered = results.filter(item => item.type === 'movie' || item.type === 'show');
      ...
      const showsCount = filtered.filter(item => item.type === 'show').length;
  ...
  renderFilteredResults() {
    ...
    const shows = this.currentResults.filter(item => item.type === 'show');
    ...
    } else if (this.currentFilter === 'show') {
      if (shows.length === 0) {
