...
class ShowDetailsView {
  ...
  render(show, onEpisodeClick, shouldShow = true) {
    ...
    this.topVisibleEpisodeId = lastWatchedEpisodeId ?? `${show.id}_s${allEpisodes[0].season}_e${allEpisodes[0].episode}`;
  ...
  renderShowHeader(show, logoContainer, yearEl) {
    if (show.logo) {
      logoContainer.innerHTML = `<img src="${show.logo}" alt="${show.title}">`;
    } else {
      logoContainer.innerHTML = `<h1 style="font-size: 6.25vh; font-weight: 900; text-transform: uppercase;">${show.title}</h1>`;
    }
