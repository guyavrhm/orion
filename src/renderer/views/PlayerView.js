...
class PlayerView {
  ...
  getDuration() {
    ...
    return this.playerState.fallbackDuration || 0;
  }
  ...
  setupSubtitlesMenu(subtitlesPromise, sessionId = this.currentSessionId) {
    ...
    Promise.all([subtitlesPromise, prefPromise]).then(([data, pref]) => {
      ...
      const subs = (data?.subtitles || []).map(sub => ({
        ...sub,
        label: LANG_MAP[sub.lang] || sub.lang
      }));

      const renderMenu = (activeUrl) => {
        const hadFocus = this.subtitlesDropdown.contains(document.activeElement);
        this.subtitlesDropdown.innerHTML = '';
        
        // "Off" option
        const offItem = document.createElement('div');
        offItem.className = `dropdown-item ${!activeUrl ? 'active' : ''}`.trim();
        offItem.tabIndex = 0;
        ...
        subs.forEach((sub) => {
          ...
          const isSelected = Boolean(activeUrl && sub.url === activeUrl);
          item.className = `dropdown-item ${isSelected ? 'active' : ''}`.trim();
          item.tabIndex = 0;
          if (isSelected) activeItem = item;
          
          item.innerText = sub.label;
      ...
      if (pref === 'off') {
        ...
        autoSub = subs.find(s => s.lang === pref) || subs[0] || null;
      } else if (subs.length > 0) {
        autoSub = subs[0];
      }
      
      renderMenu(autoSub ? autoSub.url : null);
  ...
  open(item, startTime = 0, hasNextEpisode = false, subtitlesPromise = null) {
    ...
    this.playerState.fallbackDuration = (Number(item?.runtime) || 0) * 60;
    if (item && item.type) {
      this.playerState.currentMovie = item.type === 'movie' ? item : null;
      this.playerState.currentShow = item.type === 'show' ? item : null;
    }
    ...
    if (this.movieTitleDisplay) this.movieTitleDisplay.innerText = item.title;
    if (this.subtitlesOverlay) this.subtitlesOverlay.innerHTML = '';
    this.playerState.pendingSubtitle = null;
    ...
    try {
      ...
      if (res && res.success && res.url) {
        if (res.subtitles && Array.isArray(res.subtitles)) {
          this.setupSubtitlesMenu(Promise.resolve({ subtitles: res.subtitles }), sessionId);
        }
  ...
  saveProgress(timestamp) {
    ...
    const runtimeMinutes = durationSec > 0 ? Math.round(durationSec / 60) : undefined;
    ...
    apiService.saveTimestamp({
      ...
      metadata: {
        ...
        ...(isShow && this.playerState.currentEpisode ? {
          ...
          episode: this.playerState.currentEpisode.episode
        } : {})
      }
    });
  }
