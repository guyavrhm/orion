...
import { formatRuntime, buildMetadataHtml, isDesktop, getMediaRequestErrorMessage } from '../utils/helpers.js';
...
export class MediaCard {
  ...
  static handleMediaClick(item, isContinue = false) {
    if (item.type === 'show') {
      ...
      const isReady = store.isMovieReady(item.id);
      if (isReady) {
        eventBus.emit('open-player', item);
      } else {
        const activeRequest = store.isPreparingOrQueued(item.id);
        if (!activeRequest) {
          apiService.enqueueMediaRequest({ movieId: item.id }).catch((err) => {
  ...
  static createCarouselCard(item, isContinue = false, onExpand = null) {
    ...
    if (isContinue && typeof item.timestamp === 'number' && item.timestamp > 0) {
      const defaultMinutes = item.type === 'show' ? 45 : 120;
    ...
    const logoHtml = item.logo 
      ...
      : `<div class="logo-text">${item.title}</div>`;
    ...
    this.updateRequestState(cardEl, fileId, item.type);
  ...
  static setupHeroCard(item) {
    ...
    // Set title
    if (item.logo) {
      heroTitle.innerHTML = `<img src="${item.logo}" alt="${item.title}">`;
    } else {
      heroTitle.innerHTML = `<div class="hero-title-text">${item.title.toUpperCase()}</div>`;
    }

    // Set descriptors
    heroMeta.innerHTML = buildMetadataHtml(item);
    heroDesc.innerText = item.description || '';
    this.updateRequestState(hero, item.id, item.type);
  ...
  static createSearchCard(item) {
    ...
    this.updateRequestState(cardEl, item.id, item.type);
      ...
      if (!isDesktop()) {
        ...
        if (item.type === 'movie' || item.type === 'show') {
          const fetchPromise = item.type === 'movie'
            ...
            : apiService.fetchShowDetails(item.id);
  ...
  static createEpisodeCard(show, ep, config, onEpisodeClick, seasonsList, episodesCarousel) {
    ...
    row.setAttribute('data-type', 'show');
    ...
    this.updateRequestState(row, `${show.id}_s${ep.season}_e${epNum}`, 'show');
    ...
    row.onclick = (e) => {
      ...
      const isReady = store.isEpisodeReady(show.id, ep.season, epNum);
      if (isReady) {
        onEpisodeClick(ep);
      } else {
        const fileId = `${show.id}_s${ep.season}_e${epNum}`;
        const activeRequest = store.isPreparingOrQueued(fileId);
        if (!activeRequest) {
          apiService.enqueueMediaRequest({
            showId: show.id,
            season: ep.season,
            episode: epNum
          }).catch((err) => {
  ...
  static updateRequestState(el, fileId, type) {
    ...
    if (isHero) {
      ...
      if (heroBtn) {
        if (type === 'show') {
          ...
          const isReady = store.isMovieReady(fileId);
          heroBtn.innerText = isReady ? 'Play' : 'Request';
        }
      }
    }

    if (this.shouldResetShowCard(fileId, type)) {
      this.resetRequestElements(el);
      return;
    }
    ...
    const isReady = (type === 'show')
      ? store.isEpisodeReady(showId, season, episode)
      : store.isMovieReady(fileId);
    ...
    if (isReady) {
      this.renderReady(targetContainer, isEpisodeRow);
    } else if (activeRequest) {
      this.renderPreparing(targetContainer, activeRequest);
    } else {
      this.renderNotRequested(targetContainer, isEpisodeRow);
    }
  }

  static isDirectRequestItem(targetContainer, isEpisodeRow) {
  ...
  static shouldResetShowCard(fileId, type) {
    return type === 'show' && !fileId.includes('_s');
  }

  static resetRequestElements(el) {
    const selectors = ['.card-request-overlay', '.card-request-badge', '.episode-request-badge', '.request-progress-spinner', '.card-request-label'];
  ...
  static renderReady(targetContainer, isEpisodeRow) {
    const existingOverlay = targetContainer.querySelector('.card-request-overlay');
    if (existingOverlay) existingOverlay.remove();
    const existingSpinner = targetContainer.querySelector('.request-progress-spinner');
    if (existingSpinner) existingSpinner.remove();
    const existingLabel = targetContainer.querySelector('.card-request-label');
    if (existingLabel) existingLabel.remove();

    let badge = targetContainer.querySelector('.card-request-badge') || targetContainer.querySelector('.episode-request-badge');
    const badgeClass = isEpisodeRow ? 'episode-request-badge ready' : 'card-request-badge ready';
  ...
  static renderPreparing(targetContainer, activeRequest) {
    const badge = targetContainer.querySelector('.card-request-badge') || targetContainer.querySelector('.episode-request-badge');
    if (badge) badge.remove();
    const existingLabel = targetContainer.querySelector('.card-request-label');
    if (existingLabel) existingLabel.remove();

    let overlay = targetContainer.querySelector('.card-request-overlay');
    if (!overlay) {
      overlay = document.createElement('div');
      overlay.className = 'card-request-overlay';
      targetContainer.appendChild(overlay);
    }

    let mask = overlay.querySelector('.card-request-grey-mask');
    if (!mask) {
      mask = document.createElement('div');
      mask.className = 'card-request-grey-mask';
    ...
    // Mask behavior: unmasks smoothly as progress increases
    const maskPercent = isQueued ? 0 : percent;

    requestAnimationFrame(() => {
      mask.style.left = `${maskPercent}%`;
    });

    const strokeColor = '#2ecc71';
    this.updateCircularSpinner(targetContainer, isQueued ? 0 : percent, strokeColor);
  }

  static updateCircularSpinner(targetContainer, progressPercent, strokeColor = '#2ecc71') {
    let spinner = targetContainer.querySelector('.request-progress-spinner');
    if (!spinner) {
      const svgNamespace = "http://www.w3.org/2000/svg";
      spinner = document.createElementNS(svgNamespace, "svg");
      spinner.setAttribute("class", "request-progress-spinner");
  ...
  static renderNotRequested(targetContainer, isEpisodeRow) {
    const existingSpinner = targetContainer.querySelector('.request-progress-spinner');
    if (existingSpinner) existingSpinner.remove();
    ...
    if (this.isDirectRequestItem(targetContainer, isEpisodeRow)) {
      let label = overlay.querySelector('.card-request-label');
      if (!label) {
        label = document.createElement('div');
        label.className = 'card-request-label';
        label.innerText = 'Request';
        overlay.appendChild(label);
      }
    } else {
      const existingLabel = overlay.querySelector('.card-request-label');
      if (existingLabel) existingLabel.remove();
    }

    let badge = targetContainer.querySelector('.card-request-badge') || targetContainer.querySelector('.episode-request-badge');
    const badgeClass = isEpisodeRow ? 'episode-request-badge not-requested' : 'card-request-badge not-requested';
    
    if (!badge) {
      badge = document.createElement('div');
      badge.className = badgeClass;
      badge.title = 'Request';
      badge.innerHTML = '<i class="fa-solid fa-circle-plus"></i>';
      targetContainer.insertBefore(badge, targetContainer.firstChild);
    } else {
      badge.className = badgeClass;
    }
