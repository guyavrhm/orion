...
import { toUserMediaRequestStatus } from '../utils/helpers.js';
import type { MediaRequestStatus } from '../types/index.js';
...
export class SseManager {
  ...
  private _handleRedisMessage(message: string): void {
    try {
      ...
      let { channel, data } = parsed;

      if (channel === 'media-request-status' && data) {
        const transformed = toUserMediaRequestStatus(data.status, data.progress);
        data = {
          ...data,
  ...
  /**
   * Helper to broadcast specific media request progress status events across all instances.
   * @param id Media identifier
   * @param status Request status string
   * @param progress Progress percentage string or number
   */
  async broadcastMediaRequestStatus(id: string, status: MediaRequestStatus, progress: string | number = '0.00'): Promise<number> {
    return publishMediaRequestStatus(id, status, progress);
  }
