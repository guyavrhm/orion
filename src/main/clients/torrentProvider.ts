...
import type { StreamInfo, ParsedTorrentCandidate, MediaType } from '../types/index.js';
...
/**
 * Normalizes an object's keys (lowercase, strips dashes/underscores) recursively
 * so nested properties can be accessed uniformly.
 */
export function flattenNormalizedKeys(rawObj: unknown, depth = 0): Record<string, unknown> {
  const norm: Record<string, unknown> = {};
  if (!rawObj || typeof rawObj !== 'object' || Array.isArray(rawObj) || depth > 3) return norm;

  for (const [k, v] of Object.entries(rawObj as Record<string, unknown>)) {
    const key = k.toLowerCase().replace(/[-_]/g, '');
    if (v !== undefined && v !== null && typeof v !== 'object') {
      if (norm[key] === undefined) norm[key] = v;
    } else if (v && typeof v === 'object' && !Array.isArray(v)) {
      const nested = flattenNormalizedKeys(v, depth + 1);
      for (const [nk, nv] of Object.entries(nested)) {
        if (norm[nk] === undefined) norm[nk] = nv;
      }
    }
  }
...
export class TorrentProviderClient {
  ...
  parsePeers(rawObj: Record<string, unknown>, textFallback?: string): number {
    // 1. Case-insensitive normalized key lookup (including nested containers)
    const norm = flattenNormalizedKeys(rawObj);
  ...
  buildProviderUrl(
    ...
    const apiKey = apiKeyOverride ?? this.getProviderApiKey(type);
   ...
   */
  getProviderTemplates(type: MediaType): string[] {
    const envProviders =
      type === 'movie'
   ...
   */
  getProviderTemplate(type: MediaType): string | null {
   ...
   */
  getProviderApiKey(type: MediaType, index = 0): string {
    const envKeys =
      (type === 'movie'
   ...
   */
  async resolveTitle(id: string, type: MediaType): Promise<string | undefined> {
  ...
  filterAndRankTorrents(streams: StreamInfo[], type: MediaType = 'movie', count = 3): StreamInfo[] {
    ...
    const compareSameTier = (a: StreamInfo, b: StreamInfo): number => {
      ...
      if (aIsHealthyH264 && !bIsHealthyH264) {
        if (a.peers < SATURATED_H264_THRESHOLD && b.peers >= 50 && b.peers >= a.peers * 5) {
          return b.peers - a.peers;
        }
        return -1;
      }
      if (bIsHealthyH264 && !aIsHealthyH264) {
        if (b.peers < SATURATED_H264_THRESHOLD && a.peers >= 50 && a.peers >= b.peers * 5) {
          return b.peers - a.peers;
        }
        return 1;
      }
  ...
  async getTorrentStreams(
    ...
    const cacheKey = type === 'show' ? `${id}:${season}:${episode}` : id;
