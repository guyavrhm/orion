...
export async function runFfs(
  ...
  const { stderr } = await execFilePromise(ffsBin, args);
...
export async function scoreLocalSubtitles(fileId: string, videoPath: string): Promise<void> {
  ...
  const refAudioPath = path.join(parsedPath.dir, `${fileId}_ref.wav`);
  const speechNpzPath = path.join(parsedPath.dir, `${fileId}_ref.npz`);
  ...
  try {
    ...
    // Extract reference audio padded from t=0 so container pre-roll/intros are preserved in speech detection
    try {
      logger.info(`Extracting timeline-aligned reference audio for speech detection: ${refAudioPath}`);
    ...
    } catch (extractAudioErr) {
      logger.warn(`Could not extract padded reference audio, falling back to direct video path:`, extractAudioErr);
    }

    const referenceSource = fs.existsSync(refAudioPath) ? refAudioPath : videoPath;
    ...
    for (const subFile of subFiles) {
      ...
      try {
        if (!serialized) {
          ...
          const score = await runFfs(ffsBin, referenceSource, subPath, '--serialize-speech');
          scores[subId] = score;
    ...
    // 4. Clean up temporary reference audio and serialized speech
    if (fs.existsSync(refAudioPath)) {
      try {
        fs.unlinkSync(refAudioPath);
      } catch (e) {
        logger.warn(`Failed to clean up reference audio file ${refAudioPath}`, e);
      }
    }
    if (fs.existsSync(speechNpzPath)) {
      try {
...
export async function processSubtitleJob(
  ...
  const { id: mediaId, type: mediaType, season, episode } = parsed;
