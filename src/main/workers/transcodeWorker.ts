...
export function probeMediaStreams(inputPath: string, preferredLangs: string[]): Promise<ProbedMediaInfo> {
  return new Promise((resolve) => {
    ...
    ffmpeg.ffprobe(inputPath, (err, metadata) => {
      ...
      try {
        // 1. Inspect Video Stream (ignoring attached pictures/cover art)
        ...
        // Web browsers universally only support 8-bit YUV 4:2:0 for direct H.264 playback (-c:v copy)
        const WEB_SAFE_PIX_FMTS = new Set(['yuv420p', 'yuvj420p']);
        const isVideoH264 = videoCodec === 'h264' && WEB_SAFE_PIX_FMTS.has(pixFmt);
        ...
        for (const targetLang of preferredLangs) {
          const index = audioStreams.findIndex((s) => {
            const lang = typeof s.tags?.language === 'string' ? s.tags.language.toLowerCase() : '';
            const title = typeof s.tags?.title === 'string' ? s.tags.title.toLowerCase() : '';
    ...
    probeMediaStreams(inputPath, preferredLangs)
      .then((probed) => {
        ...
        const videoOptions: string[] = probed.isVideoH264
          ? ['-c:v copy', '-bsf:v h264_mp4toannexb', '-threads 1']
          : [
              '-c:v libx264',
              '-preset veryfast',
              '-crf 22',
        ...
        if (!probed.isVideoH264) {
          ...
          if (fileId) {
            publishMediaRequestStatus(fileId, MEDIA_REQUEST_STATUS.PROCESSING, '0.00').catch((err) => {
        ...
        const proc = ffmpeg(inputPath)
          ...
          .on('progress', (progress: { percent?: number }) => {
            ...
            if (percent > lastBroadcastPercent && percent <= 100 && (now - lastBroadcastTime >= 1500 || percent === 100)) {
              ...
              publishMediaRequestStatus(fileId, MEDIA_REQUEST_STATUS.PROCESSING, percent.toFixed(2)).catch((err) => {
...
export async function processTranscodeJob(
  ...
  try {
    ...
    if (parsed) {
      const cached = metadataRepo.getCachedMetadata(parsed.id);
  ...
  try {
    ...
    if (dirs.hlsDir && fs.existsSync(dirs.hlsDir)) {
      try {
        fs.rmSync(dirs.hlsDir, { recursive: true, force: true });
        logger.info(`Purged partial HLS chunks for failed transcode: ${fileId}`);
      } catch (cleanupErr) {
        logger.warn(`Failed to clean up partial HLS directory for ${fileId}:`, cleanupErr);
      }
    }
...
export function createTranscodeWorker(
  queueName: string = QUEUE_NAMES.TRANSCODE_FAST,
  concurrency: number = WORKER_CONCURRENCY.TRANSCODE_FAST,
