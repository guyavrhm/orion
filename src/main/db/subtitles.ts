...
import type { SubtitleTrack, SubtitlePreference } from '../types/index.js';

const getSubtitlePreferenceStmt = db.prepare('SELECT subtitle_lang FROM subtitle_preferences WHERE id = ?');
const saveSubtitlePreferenceStmt = db.prepare(`
  INSERT INTO subtitle_preferences (id, subtitle_lang)
  VALUES (?, ?)
  ON CONFLICT(id) DO UPDATE SET subtitle_lang=excluded.subtitle_lang
`);
...
export function getSubtitlePreference(mediaId: string): string | null {
  try {
    const row = getSubtitlePreferenceStmt.get(mediaId) as Pick<SubtitlePreference, 'subtitle_lang'> | undefined;
 ...
 * @returns List of canonical SubtitleTrack objects
 */
export function getLocalSubtitles(fileId: string): SubtitleTrack[] {
    ...
    if (dirs && fs.existsSync(dirs.subtitlesDir)) {
      ...
      for (const lang of SUBTITLE_LANGS) {
        ...
        if (langFiles.length > 0) {
          // Sort by score descending (treat missing scores as lowest quality)
          langFiles.sort((a, b) => {
            const idA = a.replace('.vtt', '');
            const idB = b.replace('.vtt', '');
            const scoreA = scores[idA] ?? -Infinity;
            const scoreB = scores[idB] ?? -Infinity;
            return scoreB - scoreA;
          });

          // Return only the highest scoring subtitle track for this language
          const bestFile = langFiles[0];
          subtitles.push({
            lang,
            url: `/stream/subtitles/${fileId}/${bestFile}`
          });
        }
      }
    }
