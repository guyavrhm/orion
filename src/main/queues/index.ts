 ...
 *
 * @param channel Logical event channel/name (e.g., 'request-status')
 ...
 *
/**
 * Publishes a real-time request status event to the Redis event channel and updates
 * active media tracking state in Redis.
 *
 * @param id Unique media or file identifier
 * @param status Status string (from MediaRequestStatus)
 * @param progress Progress percentage string or number
 * @returns Number of subscribers notified
 */
export async function publishMediaRequestStatus(
  id: string,
  status: MediaRequestStatus,
  progress: string | number = '0.00'
): Promise<number> {
  if (
    ...
    status === MEDIA_REQUEST_STATUS.FAILED ||
    status === MEDIA_REQUEST_STATUS.REMOVED
  ) {
  ...
  return publishEvent('media-request-status', {
    id,
    status,
    progress: String(progress)
  });
